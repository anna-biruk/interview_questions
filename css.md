...
### Какие существуют различные методы клиринга и какие из них подходят для какого контекста?

- Пустой метод `div` - стиль `<div style="clear:both;"></div>`.
- Метод Clearfix - см. Класс `.clearfix` выше.
- `overflow: auto` или `overflow: hidden` метод - родительский элемент устанавливает новый контекст форматирования блока и расширяет его, чтобы содержать его плавающие дочерние элементы.

В больших проектах я бы написал служебный класс 
`.clearfix` и использовал бы его там, где мне это нужно. `overflow: hidden` может обрезать дочерние элементы, если они выше, чем родитель, и не очень идеальны.
